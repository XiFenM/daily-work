...
课程素材已经到位并完成全量编号映射。第一至四章共 27 节课已走完阶段 1–6 的章节范围流程：

- 27 个视频完成 SHA-256、FFprobe、画面抽样和压缩副本校验；
- 27 节完成 ZenMux 视频理解、规范化证据和本地门禁；
- 27 份单课笔记与 4 份章节综述已完成；
- 概念词表和关系图已完成第四章校准；
- 原始响应、失败尝试、规范化证据、请求 ID、Prompt 和压缩副本均已审计到 `manifest.json`。

第四章共发起 11 次到达服务端的理解请求，8 个课次均取得可用结果；`04-02` 两次输出分别因顶层 `complexity` 为 `null`、实验对象缺少自身 `evidenceIds` 被拒绝，`04-08` 一次输出因空转移状态模型与 `state_transition` 不一致被拒绝。人工 QA 还校正了 `04-05` 的数量级换算，并把 `04-06` 漏转录条件行的代码证据降为 `partial`。失败与校正均保留审计记录，未通过门禁的结果不参与综合。下一步从第五章 `05-01` 开始。
