...
课程素材已经到位并完成全量编号映射。第一至六章共 41 节课已走完阶段 1–6 的章节范围流程：

- 40 个视频完成 SHA-256、FFprobe、画面抽样和压缩副本校验，`06-08` 文本完成哈希与段落证据校验；
- 40 个视频完成 ZenMux 视频理解、规范化证据和本地门禁，1 篇文本完成本地规范化证据；
- 41 份单课笔记与 6 份章节综述已完成；
- 概念词表和关系图已完成第六章校准；
- 原始响应、失败尝试、规范化证据、请求 ID、Prompt 和压缩副本均已审计到 `manifest.json`。

第六章 7 个视频各发起 1 次到达服务端的理解请求，均一次取得可用结果；文本课 `06-08` 没有调用视频模型。人工 QA 把 `06-03` 前、中、后序遍历拆成三套独立状态模型，修正 `06-06` 自定义比较器被模型写反的问题，把 `06-07` 不完整的屏幕代码降为 `partial` 并纠正复杂度情形与规模变量口径；本地测试或编译状态与课程现场 Accepted、运行画面分别记录。下一步从第七章 `07-01` 开始。
