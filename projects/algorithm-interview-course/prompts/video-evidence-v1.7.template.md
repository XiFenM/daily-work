...
- 目录原始标题：`{{TITLE_RAW}}`
- 目录规范标题：`{{TITLE_NORMALIZED}}`
- 目录与 FFprobe 时长：`{{EXPECTED_DURATION_OR_UNKNOWN}}`
- **视频实测总时长：`{{ACTUAL_DURATION_SECONDS}}` 秒，即 `{{ACTUAL_DURATION_MS}}` 毫秒**
- 视频资产 ID：`{{VIDEO_ASSET_ID}}`
- 相邻课次，仅用于定位：`{{NEIGHBOR_LESSONS}}`
- 可核查的关系候选：`{{CANDIDATE_RELATED_LESSONS}}`
...
2. **任何时间戳都不得大于 `{{ACTUAL_DURATION_MS}}`。**
3. 压缩副本保持原时长；不得使用其他版本课程、网页播放器或记忆中的时间轴。
4. 无法可靠定位时填 `null`，不要猜测。
5. 输出前必须检查最后一个 `endMs <= {{ACTUAL_DURATION_MS}}`。
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "{{VIDEO_ASSET_ID}}"`；
- 所有时间戳不超过 `{{ACTUAL_DURATION_MS}}`；
