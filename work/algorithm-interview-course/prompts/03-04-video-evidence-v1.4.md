...
- 课次 ID：`03-04`
- 章节：`03 数组中的问题其实最常见`
- 目录原始标题：`即使简单的问题，也有很多优化的思路`
- 目录规范标题：`即使简单的问题，也有很多优化的思路`
- 目录与 FFprobe 时长：`15:30（目录标称）；930.48 秒（FFprobe）`
- **视频实测总时长：`930.48` 秒，即 `930480` 毫秒**
- 视频资产 ID：`asset:video-03-04`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "03-03",
  ...
  {
    "lessonId": "03-05",
    "title": "三路快排 partition 思路的应用 Sort Color"
  }
]`
- 可核查的关系候选：`[
  {
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-03-04"`；
- 所有时间戳不超过 `930480`；
