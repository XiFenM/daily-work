...
- 课次 ID：`04-03`
- 章节：`04 查找表相关问题`
- 目录原始标题：`set和map不同底层实现的区别`
- 目录规范标题：`set 和 map 不同底层实现的区别`
- 目录与 FFprobe 时长：`19:00（目录标称）；1140.12 秒（FFprobe）`
- **视频实测总时长：`1140.12` 秒，即 `1140120` 毫秒**
- 视频资产 ID：`asset:video-04-03`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "04-02",
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-04-03"`；
- 所有时间戳不超过 `1140120`；
