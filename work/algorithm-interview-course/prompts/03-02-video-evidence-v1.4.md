...
- 课次 ID：`03-02`
- 章节：`03 数组中的问题其实最常见`
- 目录原始标题：`改变变量定义，依然可以写出正确的算法`
- 目录规范标题：`改变变量定义，依然可以写出正确的算法`
- 目录与 FFprobe 时长：`12:06（目录标称）；726.48 秒（FFprobe）`
- **视频实测总时长：`726.48` 秒，即 `726480` 毫秒**
- 视频资产 ID：`asset:video-03-02`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "03-01",
  ...
  {
    "lessonId": "03-03",
    "title": "在 LeetCode 上解决第一个问题 Move Zeros"
  }
]`
- 可核查的关系候选：`[
  {
    "from": "lesson:03-01",
    "fromTitle": "从二分查找法看如何写出正确的程序",
    "to": "lesson:03-02",
    "toTitle": "改变变量定义，依然可以写出正确的算法",
    "type": "extends",
    "status": "provisional",
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-03-02"`；
- 所有时间戳不超过 `726480`；
