...
- 目录原始标题：`优先队列`
- 目录规范标题：`优先队列`
- 目录与 FFprobe 时长：`11:09（目录标称）；669.04 秒（FFprobe）`
- **视频实测总时长：`669.04` 秒，即 `669040` 毫秒**
- 视频资产 ID：`asset:video-06-06`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "06-05",
  ...
  {
    "lessonId": "06-07",
    "title": "优先队列相关的算法问题 Top K Frequent Elements"
  }
]`
- 可核查的关系候选：`[
  {
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-06-06"`；
- 所有时间戳不超过 `669040`；
