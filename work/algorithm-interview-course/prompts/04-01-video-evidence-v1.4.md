...
- 课次 ID：`04-01`
- 章节：`04 查找表相关问题`
- 目录原始标题：`set的使用 Intersection of Two Arrays`
- 目录规范标题：`set 的使用 Intersection of Two Arrays`
- 目录与 FFprobe 时长：`11:12（目录标称）；675.283 秒（FFprobe）`
- **视频实测总时长：`675.283` 秒，即 `675283` 毫秒**
- 视频资产 ID：`asset:video-04-01`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "04-02",
    "title": "map 的使用 Intersection of Two Arrays II"
  }
]`
- 可核查的关系候选：`[
  {
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-04-01"`；
- 所有时间戳不超过 `675283`；
