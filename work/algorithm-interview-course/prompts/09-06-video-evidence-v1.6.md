...
- 课次 ID：`09-06`
- 章节：`09 动态规划基础`
- 目录原始标题：`0-1背包问题的优化和变种`
- 目录规范标题：`0-1 背包问题的优化和变种`
- 目录与 FFprobe 时长：`20:18（目录标称）；1218.36 秒（FFprobe）`
- **视频实测总时长：`1218.36` 秒，即 `1218360` 毫秒**
- 视频资产 ID：`asset:video-09-06`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "09-05",
    "title": "0-1 背包问题"
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-09-06"`；
- 所有时间戳不超过 `1218360`；
