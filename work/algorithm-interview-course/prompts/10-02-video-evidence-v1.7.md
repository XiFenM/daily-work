# Video Evidence Extraction Prompt v1.7
...
- 课次 ID：`10-02`
- 章节：`10 贪心算法`
- 目录原始标题：`贪心算法与动态规划的关系 Non-overlapping Intervals`
- 目录规范标题：`贪心算法与动态规划的关系 Non-overlapping Intervals`
- 目录与 FFprobe 时长：`17:58（目录标称）；1078.68 秒（FFprobe）`
- **视频实测总时长：`1078.68` 秒，即 `1078680` 毫秒**
- 视频资产 ID：`asset:video-10-02`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "10-01",
    "title": "贪心基础 Assign Cookies"
  },
  {
    "lessonId": "10-03",
    "title": "贪心选择性质的证明"
...
- 可核查的关系候选：`[
  {
    ...
    "fromTitle": "0-1 背包问题",
    "to": "lesson:10-02",
    "toTitle": "贪心算法与动态规划的关系 Non-overlapping Intervals",
    "type": "contrasts_with",
    "status": "provisional",
  ...
  {
    "from": "lesson:10-02",
    "fromTitle": "贪心算法与动态规划的关系 Non-overlapping Intervals",
    "to": "lesson:10-03",
    "toTitle": "贪心选择性质的证明",
    "type": "extends",
    "status": "provisional",
...
12. `correctness.method` 只能是 `not_applicable`、`loop_invariant`、`recursive_contract`、`induction`、`state_transition`、`exchange_argument`、`stays_ahead`、`experimental_validation`、`other`、`unknown`。
...
1. 每个贪心解法必须分开记录候选集合、候选排序或扫描顺序、当前选择规则、可行性检查、提交选择后如何更新状态，以及最终目标值从哪里得到；不能只写“排序后贪心”。第 10 章中 `stage` 为 `intermediate` 或 `optimized` 的解法，以及任何含屏幕代码的解法，都必须链接至少一个含变量/区域和转移的 `stateModels` 项。
2. 候选排序必须精确记录主键、升序/降序、比较器中的严格或非严格关系，以及并列主键时的 tie-break。含代码或处于 `intermediate`/`optimized` 阶段的排序解法，要用 `stateModels.variables[].role: "candidate_order"` 标识候选顺序，并在其 `meaning`/`updateRule` 中记录可见排序语义。视频没有说明 tie-break、稳定排序或相等元素顺序时，不得套用常见实现，必须在 `uncertainties` 中明确 tie-break 缺口。
3. 若代码与口述使用不同排序键、方向或 tie-break，分别记录冲突和证据。比较器代码中的 `<`、`<=`、`>`、`>=`、二级键和返回值都是关键 token；关键 token 不清时，代码使用 `partial` 或 `uncertain`，不能补写。
4. 贪心循环的状态模型应按视频记录“已选择前缀”“尚未处理候选”“当前可行性边界/容量/端点”“当前目标值”和扫描指针。只有视频明确给出每轮开始时保持的性质，才把它写成 `invariant: course_direct`；代码执行顺序本身不等于正确性不变量。
5. 区间问题必须记录端点语义：闭区间、开区间、半开区间，端点相等时算冲突还是可衔接，以及排序键是起点还是终点。若视频与题面未说明端点约定，相关比较与反例结论必须保留不确定性，不能按 LeetCode 记忆补齐。
6. “贪心选择性质”与“最优子结构”必须拆开：前者说明存在一个最优解可采用当前贪心选择，后者说明作出该选择后的剩余问题仍可由同类最优子问题刻画。视频只讲其中一个时，只记录该部分，不能把两个概念合并成完整证明。
7. `correctness.method: "exchange_argument"` 只用于视频展示交换论证时。若要标记 `correctness.completeness: "complete"`，必须分别提供 `greedy_choice`（明确贪心选择）、`exchange_step`（把任意最优解的首个分歧替换为贪心选择且目标不变差）、`optimal_substructure`（交换后的剩余问题仍保持所需最优结构）和 `postcondition`（重复交换得到贪心解）的直接视频证据义务；缺任一项必须标为 `partial`。
8. `correctness.method: "stays_ahead"` 只用于视频明确比较贪心前缀与任意可行/最优前缀的领先性时。若要标记 `complete`，必须有 `initialization`、`prefix_dominance`、`preservation`、`termination` 和 `postcondition` 五类直接证据义务；只展示若干前缀例子不能冒充领先性证明。
9. 循环不变量或归纳法只有满足其各自完整义务时才可标记 `complete`。`state_transition`、样例验证、平台 Accepted、直觉、图示或“看起来最优”不能证明全局最优；课程没有给出完整证明时，`correctness.completeness` 必须是 `partial`，并在 `uncertainties` 中列出缺失义务，不得用模型常识补齐 `course_direct`。
10. 反例只能否定它实际击中的具体规则及其前提，例如“按最早开始”“按最短长度”或“按最大局部收益”。不得从一个反例推出“所有贪心都失败”“只能用动态规划”，也不得把端点、正负性或 tie-break 不同的变体混为同一规则。
11. 贪心与动态规划同时出现时，分别建立 `solutionProgression` 项并绑定各自状态、代码、复杂度与正确性证据。只按视频比较状态维度、是否枚举选择、是否保留历史、时间/空间代价和适用前提；不得根据教材知识补写“贪心更快”或“DP 总能求解”。
12. 贪心选择一旦提交是否可撤销、是否回看已选前缀、失败候选是否永久丢弃，都必须来自视频。若实现包含回溯、重新排序或替换已选项，要按真实控制流记录，不能仍概括为单向贪心。
13. 排序后的线性扫描、优先队列选择和不排序的局部选择是不同实现；分别记录候选产生方式、数据结构语义、每次弹出/跳过/接受的条件，以及它们是否实现同一条有证据的贪心规则。
14. 贪心复杂度必须绑定到具体实现并拆分排序、扫描、堆操作或其他阶段。只有视频明确给出时才能把 `O(n log n) + O(n)` 归一化为 `O(n log n)`；不能忽略排序或根据库函数名擅自补复杂度。
15. 同一目标下的不同排序规则、不同 tie-break 或不同端点语义应视为不同候选规则；只有视频明确说它们等价时才能合并。课程只描述但未实现/证明的规则可保留为无代码的 `observation` 或 `alternative`，不要为通过门禁补造状态或代码。
...
"requestId": null,
"promptVersion": "video-evidence-v1.7",
"generatedAt": null
}
}
...
`completeness` 只能是 `complete`、`partial`、`not_applicable`、`unknown`。存在解法、正确性主张或状态模型时只能使用 `complete` 或 `partial`；课程未给出完整证明时必须使用 `partial`。`method: "not_applicable"` 与 `completeness: "not_applicable"` 必须同时出现。`method` 只能是 `not_applicable`、`loop_invariant`、`recursive_contract`、`induction`、`state_transition`、`exchange_argument`、`stays_ahead`、`experimental_validation`、`other`、`unknown`。

`phase` 只能是 `initialization`、`preservation`、`termination`、`postcondition`、`boundary_safety`、`greedy_choice`、`exchange_step`、`prefix_dominance`、`optimal_substructure`。同一阶段可以有多个义务（例如不同分支各自的保持性）。使用 `loop_invariant` 且标记 `complete` 时，四个阶段 `initialization`、`preservation`、`termination`、`postcondition` 都必须有直接视频证据；状态模型的 `invariant` 本身也必须是 `course_direct` 且引用视频证据。使用 `state_transition` 时，`stateModelIds` 必须引用至少一个 `transitions` 非空的状态模型；若无法从视频中提取任何转移，则必须使用 `unknown`。交换论证与领先性证明的完整义务见“贪心算法专项证据规则”；不满足时必须标记 `partial`。
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-10-02"`；
- 所有时间戳不超过 `1078680`；
...
- 第 10 章每个 `intermediate`/`optimized` 或含代码的解法都引用含变量/区域与转移的状态模型；候选主排序键、方向、tie-break、可行性检查和提交后的更新只写视频可见内容；
- 区间端点的开闭和相等端点语义没有被默认补齐；反例只否定其实际覆盖的具体规则和前提；
- `correctness.completeness: "complete"` 只用于已满足所选证明方法全部直接证据义务的情况；否则为 `partial`，缺口已写入 `uncertainties`；
- 贪心选择性质与最优子结构分开记录，贪心与动态规划的对照没有跨解法混用状态、代码、复杂度或证明。
