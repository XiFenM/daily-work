...
- 目录原始标题：`什么是动态规划`
- 目录规范标题：`什么是动态规划`
- 目录与 FFprobe 时长：`20:27（目录标称）；1230.96 秒（FFprobe）`
- **视频实测总时长：`1230.96` 秒，即 `1230960` 毫秒**
- 视频资产 ID：`asset:video-09-01`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "09-02",
...
- 可核查的关系候选：`[
  ...
  },
  {
    ...
    "fromTitle": "什么是动态规划",
    "to": "lesson:09-02",
    "toTitle": "第一个动态规划问题 Climbing Stairs",
    "type": "applies",
    "status": "provisional",
    "priorKind": "non_evidence_candidate_for_checking"
  },
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-09-01"`；
- 所有时间戳不超过 `1230960`；
