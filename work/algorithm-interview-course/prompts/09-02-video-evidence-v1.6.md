...
- 课次 ID：`09-02`
- 章节：`09 动态规划基础`
- 目录原始标题：`第一个动态规划问题 Climbing Stairs`
- 目录规范标题：`第一个动态规划问题 Climbing Stairs`
- 目录与 FFprobe 时长：`14:02（目录标称）；842.68 秒（FFprobe）`
- **视频实测总时长：`842.68` 秒，即 `842680` 毫秒**
- 视频资产 ID：`asset:video-09-02`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "09-01",
    "title": "什么是动态规划"
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-09-02"`；
- 所有时间戳不超过 `842680`；
