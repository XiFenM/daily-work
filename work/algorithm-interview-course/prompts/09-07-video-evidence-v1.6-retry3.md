...
- 课次 ID：`09-07`
- 章节：`09 动态规划基础`
- 目录原始标题：`面试中的0-1背包问题 Partition Equal Subset Sum`
- 目录规范标题：`面试中的 0-1 背包问题 Partition Equal Subset Sum`
- 目录与 FFprobe 时长：`27:34（目录标称）；1654.653968 秒（FFprobe）`
- **视频实测总时长：`1654.653968` 秒，即 `1654654` 毫秒**
- 视频资产 ID：`asset:video-09-07`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "09-06",
    "title": "0-1 背包问题的优化和变种"
  },
  {
    "lessonId": "09-08",
    "title": "LIS 问题 Longest Increasing Subsequence"
  }
]`
- 可核查的关系候选：`[
  {
    "from": "lesson:09-06",
    "fromTitle": "0-1 背包问题的优化和变种",
    "to": "lesson:09-07",
    "toTitle": "面试中的 0-1 背包问题 Partition Equal Subset Sum",
    "type": "applies",
    "status": "provisional",
...
"titleObserved": null,
"contentKind": "unknown",
"actualDurationSeconds": 1654.653968,
"timeline": [],
"learningObjectives": [],
"concepts": [],
"problem": null,
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-09-07"`；
- 所有时间戳不超过 `1654654`；
...
前两次响应均因把末段终点四舍五入为 `1655000` ms 而被拒绝。当前视频的安全向下取整终点固定为 **`1654653` ms**。最后一个 `timeline` 段和最后一条 `evidence` 的 `endMs` 必须逐字写成 `1654653`；任何对象都禁止出现大于 `1654653` 的毫秒值。输出前搜索整个 JSON 中所有 `startMs`/`endMs` 数字并逐一检查。请重新独立分析完整视频，不引用前次响应。
