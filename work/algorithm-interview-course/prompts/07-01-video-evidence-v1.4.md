...
- 目录原始标题：`二叉树天然的递归结构`
- 目录规范标题：`二叉树天然的递归结构`
- 目录与 FFprobe 时长：`15:58（目录标称）；961.24 秒（FFprobe）`
- **视频实测总时长：`961.24` 秒，即 `961240` 毫秒**
- 视频资产 ID：`asset:video-07-01`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "07-02",
    "title": "一个简单的二叉树问题引发的血案 Invert Binary Tree"
  }
]`
- 可核查的关系候选：`[
  {
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-07-01"`；
- 所有时间戳不超过 `961240`；
