...
- 课次 ID：`03-05`
- 章节：`03 数组中的问题其实最常见`
- 目录原始标题：`三路快排partition思路的应用 Sort Color`
- 目录规范标题：`三路快排 partition 思路的应用 Sort Color`
- 目录与 FFprobe 时长：`22:26（目录标称）；1346.64 秒（FFprobe）`
- **视频实测总时长：`1346.64` 秒，即 `1346640` 毫秒**
- 视频资产 ID：`asset:video-03-05`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "03-04",
  ...
  {
    "lessonId": "03-06",
    "title": "对撞指针 Two Sum II - Input Array is Sorted"
  }
]`
- 可核查的关系候选：`[
  {
    "from": "lesson:03-05",
    "fromTitle": "三路快排 partition 思路的应用 Sort Color",
    "to": "lesson:01-01",
    "toTitle": "算法面试不仅仅是正确的回答问题",
    "type": "revisits",
    "status": "provisional",
  ...
  {
    "from": "lesson:03-04",
    "fromTitle": "即使简单的问题，也有很多优化的思路",
    "to": "lesson:03-05",
    "toTitle": "三路快排 partition 思路的应用 Sort Color",
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-03-05"`；
- 所有时间戳不超过 `1346640`；
