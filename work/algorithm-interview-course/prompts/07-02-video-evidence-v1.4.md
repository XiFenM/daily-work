...
- 课次 ID：`07-02`
- 章节：`07 二叉树和递归`
- 目录原始标题：`一个简单的二叉树问题引发的血案 Invert Binary Tree`
- 目录规范标题：`一个简单的二叉树问题引发的血案 Invert Binary Tree`
- 目录与 FFprobe 时长：`09:08（目录标称）；549 秒（FFprobe）`
- **视频实测总时长：`549` 秒，即 `549000` 毫秒**
- 视频资产 ID：`asset:video-07-02`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "07-01",
    "title": "二叉树天然的递归结构"
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-07-02"`；
- 所有时间戳不超过 `549000`；
