...
上一响应因末段时间边界四舍五入失败：视频实测上限是 `1654654` ms，但末段曾写成 `1655000` ms。请重新独立分析完整视频，并在输出前逐项检查 `timeline` 与 `evidence`：所有非空 `startMs/endMs` 都必须 `<= 1654654`，最后一段建议精确以 `1654654` 结束。不得引用上一响应或为了通过门禁删除真实内容。
