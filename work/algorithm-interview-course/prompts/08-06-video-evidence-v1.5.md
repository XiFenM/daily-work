...
- 课次 ID：`08-06`
- 章节：`08 递归和回溯法`
- 目录原始标题：`二维平面上的回溯法 Word Search`
- 目录规范标题：`二维平面上的回溯法 Word Search`
- 目录与 FFprobe 时长：`21:45（目录标称）；1305.16 秒（FFprobe）`
- **视频实测总时长：`1305.16` 秒，即 `1305160` 毫秒**
- 视频资产 ID：`asset:video-08-06`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "08-05",
    "title": "回溯法解决组合问题的优化"
  ...
  {
    "lessonId": "08-07",
    "title": "flood fill 算法，一类经典问题 Number of Islands"
  }
]`
- 可核查的关系候选：`[
  {
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-08-06"`；
- 所有时间戳不超过 `1305160`；
