...
- 课次 ID：`04-02`
- 章节：`04 查找表相关问题`
- 目录原始标题：`map的使用 Intersection of Two Arrays II`
- 目录规范标题：`map 的使用 Intersection of Two Arrays II`
- 目录与 FFprobe 时长：`13:05（目录标称）；785.96 秒（FFprobe）`
- **视频实测总时长：`785.96` 秒，即 `785960` 毫秒**
- 视频资产 ID：`asset:video-04-02`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "04-01",
    "title": "set 的使用 Intersection of Two Arrays"
  },
  {
    "lessonId": "04-03",
    "title": "set 和 map 不同底层实现的区别"
  }
]`
- 可核查的关系候选：`[
  {
    "from": "lesson:04-01",
    "fromTitle": "set 的使用 Intersection of Two Arrays",
    "to": "lesson:04-02",
    "toTitle": "map 的使用 Intersection of Two Arrays II",
    "type": "extends",
    "status": "provisional",
  ...
  {
    "from": "lesson:04-02",
    "fromTitle": "map 的使用 Intersection of Two Arrays II",
    "to": "lesson:04-03",
    "toTitle": "set 和 map 不同底层实现的区别",
    "type": "extends",
    "status": "provisional",
    "priorKind": "non_evidence_candidate_for_checking"
  }
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-04-02"`；
- 所有时间戳不超过 `785960`；
