...
- 课次 ID：`07-07`
- 章节：`07 二叉树和递归`
- 目录原始标题：`二分搜索树中的问题 Lowest Common Ancestor of a Binary Search Tree`
- 目录规范标题：`二分搜索树中的问题 Lowest Common Ancestor of a Binary Search Tree`
- 目录与 FFprobe 时长：`17:46（目录标称）；1070.44 秒（FFprobe）`
- **视频实测总时长：`1070.44` 秒，即 `1070440` 毫秒**
- 视频资产 ID：`asset:video-07-07`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "07-06",
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-07-07"`；
- 所有时间戳不超过 `1070440`；
