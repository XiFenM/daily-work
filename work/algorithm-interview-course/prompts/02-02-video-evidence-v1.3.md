...
- 目录原始标题：`对数据规模有一个概念`
- 目录规范标题：`对数据规模有一个概念`
- 目录与 FFprobe 时长：`11:26（目录标称）；686.289 秒（FFprobe）`
- **视频实测总时长：`686.289` 秒，即 `686289` 毫秒**
- 视频资产 ID：`asset:video-02-02`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "02-01",
    "title": "究竟什么是大 O（Big O）"
  },
  {
    "lessonId": "02-03",
    "title": "简单的复杂度分析"
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-02-02"`；
- 所有时间戳不超过 `686289`；
