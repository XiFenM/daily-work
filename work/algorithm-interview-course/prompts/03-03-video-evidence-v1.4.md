...
- 课次 ID：`03-03`
- 章节：`03 数组中的问题其实最常见`
- 目录原始标题：`在LeetCode上解决第一个问题 Move Zeros`
- 目录规范标题：`在 LeetCode 上解决第一个问题 Move Zeros`
- 目录与 FFprobe 时长：`13:27（目录标称）；807.52 秒（FFprobe）`
- **视频实测总时长：`807.52` 秒，即 `807520` 毫秒**
- 视频资产 ID：`asset:video-03-03`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "03-02",
    "title": "改变变量定义，依然可以写出正确的算法"
  },
  {
    "lessonId": "03-04",
    "title": "即使简单的问题，也有很多优化的思路"
  }
]`
- 可核查的关系候选：`[
  {
    "from": "lesson:03-03",
    "fromTitle": "在 LeetCode 上解决第一个问题 Move Zeros",
    "to": "lesson:03-04",
    "toTitle": "即使简单的问题，也有很多优化的思路",
    "type": "extends",
    "status": "provisional",
...
`kind` 只能是 `search_interval`、`partition`、`two_pointer`、`sliding_window`、`other`。`invariant`、`transitions[].preserves` 和 `termination` 是 sourced statement 或 `null`。变量、区域和转移每项都必须引用证据。
...
`phase` 只能是 `initialization`、`preservation`、`termination`、`postcondition`、`boundary_safety`。使用 `loop_invariant` 时，四个阶段 `initialization`、`preservation`、`termination`、`postcondition` 都必须有直接证据。
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-03-03"`；
- 所有时间戳不超过 `807520`；
