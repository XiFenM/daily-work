...
- 课次 ID：`09-09`
- 章节：`09 动态规划基础`
- 目录原始标题：`LCS，最短路，求动态规划的具体解以及更多`
- 目录规范标题：`LCS、最短路、求动态规划的具体解以及更多`
- 目录与 FFprobe 时长：`21:00（目录标称）；1264.76771 秒（FFprobe）`
- **视频实测总时长：`1264.76771` 秒，即 `1264768` 毫秒**
- 视频资产 ID：`asset:video-09-09`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "09-08",
...
- 可核查的关系候选：`[
  {
    "from": "lesson:09-08",
    "fromTitle": "LIS 问题 Longest Increasing Subsequence",
    "to": "lesson:09-09",
    "toTitle": "LCS、最短路、求动态规划的具体解以及更多",
    "type": "extends",
    "status": "provisional",
...
"titleObserved": null,
"contentKind": "unknown",
"actualDurationSeconds": 1264.76771,
"timeline": [],
"learningObjectives": [],
"concepts": [],
"problem": null,
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-09-09"`；
- 所有时间戳不超过 `1264768`；
...
上一响应因把末段终点四舍五入为 `1265000` ms 而被拒绝。当前视频的安全向下取整终点固定为 **`1264767` ms**。最后一个 `timeline` 段和最后一条 `evidence` 的 `endMs` 必须逐字写成 `1264767`；任何对象都禁止出现大于 `1264767` 的毫秒值。输出前搜索整个 JSON 中所有 `startMs`/`endMs` 数字并逐一检查。请重新独立分析完整视频，不引用前次响应。
