...
- 课次 ID：`06-05`
- 章节：`06 栈，队列，优先队列`
- 目录原始标题：`BFS和图的最短路径 Perfect Squares`
- 目录规范标题：`BFS 和图的最短路径 Perfect Squares`
- 目录与 FFprobe 时长：`21:20（目录标称）；1280.88 秒（FFprobe）`
- **视频实测总时长：`1280.88` 秒，即 `1280880` 毫秒**
- 视频资产 ID：`asset:video-06-05`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "06-04",
    "title": "队列的典型应用 Binary Tree Level Order Traversal"
  },
  {
    "lessonId": "06-06",
    "title": "优先队列"
...
- 可核查的关系候选：`[
  {
    "from": "lesson:06-04",
    "fromTitle": "队列的典型应用 Binary Tree Level Order Traversal",
    "to": "lesson:06-05",
    "toTitle": "BFS 和图的最短路径 Perfect Squares",
    "type": "extends",
    "status": "provisional",
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-06-05"`；
- 所有时间戳不超过 `1280880`；
