...
- 课次 ID：`05-04`
- 章节：`05 在链表中穿针引线`
- 目录原始标题：`复杂的穿针引线 Swap Nodes in Pairs`
- 目录规范标题：`复杂的穿针引线 Swap Nodes in Pairs`
- 目录与 FFprobe 时长：`13:30（目录标称）；810.72 秒（FFprobe）`
- **视频实测总时长：`810.72` 秒，即 `810720` 毫秒**
- 视频资产 ID：`asset:video-05-04`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "05-03",
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-05-04"`；
- 所有时间戳不超过 `810720`；
