...
- 目录原始标题：`0-1背包问题`
- 目录规范标题：`0-1 背包问题`
- 目录与 FFprobe 时长：`32:37（目录标称）；1957.92 秒（FFprobe）`
- **视频实测总时长：`1957.92` 秒，即 `1957920` 毫秒**
- 视频资产 ID：`asset:video-09-05`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "09-04",
...
2. 第 9 章中每个不是纯观察的 `solutionProgression` 项都必须引用至少一个 `stateModels` 项。被解法引用的状态模型必须含有至少一个有证据的变量或区域，并至少含有一个有证据的转移；若视频只提到某算法名、没有给出可辨的状态和转移，不要创建虚假的解法项。
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-09-05"`；
- 所有时间戳不超过 `1957920`；
- 所有代码 `verification` 为 `not_run`；
- `loop_invariant` 的状态模型和四阶段义务齐全。
- 第 9 章每个非观察解法都引用含变量/区域与转移的状态模型；
