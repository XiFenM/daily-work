...
- 课次 ID：`03-01`
- 章节：`03 数组中的问题其实最常见`
- 目录原始标题：`从二分查找法看如何写出正确的程序`
- 目录规范标题：`从二分查找法看如何写出正确的程序`
- 目录与 FFprobe 时长：`15:27（目录标称）；930.52 秒（FFprobe）`
- **视频实测总时长：`930.52` 秒，即 `930520` 毫秒**
- 视频资产 ID：`asset:video-03-01`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "03-02",
...
`kind` 只能是 `search_interval`、`partition`、`two_pointer`、`sliding_window`、`other`。`invariant` 和 `termination` 是 sourced statement 或 `null`。变量、区域和转移每项都必须引用证据。
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-03-01"`；
- 所有时间戳不超过 `930520`；
