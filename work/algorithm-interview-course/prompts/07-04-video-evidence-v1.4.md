...
- 课次 ID：`07-04`
- 章节：`07 二叉树和递归`
- 目录原始标题：`注意递归的终止条件 Path Sum`
- 目录规范标题：`注意递归的终止条件 Path Sum`
- 目录与 FFprobe 时长：`12:04（目录标称）；724.2 秒（FFprobe）`
- **视频实测总时长：`724.2` 秒，即 `724200` 毫秒**
- 视频资产 ID：`asset:video-07-04`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "07-03",
    "title": "不会翻转二叉树的大神"
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-07-04"`；
- 所有时间戳不超过 `724200`；
