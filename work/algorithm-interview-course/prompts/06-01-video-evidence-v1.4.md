...
- 课次 ID：`06-01`
- 章节：`06 栈，队列，优先队列`
- 目录原始标题：`栈的基础应用 Valid Parentheses`
- 目录规范标题：`栈的基础应用 Valid Parentheses`
- 目录与 FFprobe 时长：`16:10（目录标称）；973.44 秒（FFprobe）`
- **视频实测总时长：`973.44` 秒，即 `973440` 毫秒**
- 视频资产 ID：`asset:video-06-01`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "06-02",
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-06-01"`；
- 所有时间戳不超过 `973440`；
