...
- 课次 ID：`10-01`
- 章节：`10 贪心算法`
- 目录原始标题：`贪心基础 Assign Cookies`
- 目录规范标题：`贪心基础 Assign Cookies`
- 目录与 FFprobe 时长：`12:12（目录标称）；735.44 秒（FFprobe）`
- **视频实测总时长：`735.44` 秒，即 `735440` 毫秒**
- 视频资产 ID：`asset:video-10-01`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "10-02",
    "title": "贪心算法与动态规划的关系 Non-overlapping Intervals"
  }
]`
- 可核查的关系候选：`[
  {
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-10-01"`；
- 所有时间戳不超过 `735440`；
