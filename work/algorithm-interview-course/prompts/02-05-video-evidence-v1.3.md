...
- 目录原始标题：`递归算法的复杂度分析`
- 目录规范标题：`递归算法的复杂度分析`
- 目录与 FFprobe 时长：`14:46（目录标称）；886.44 秒（FFprobe）`
- **视频实测总时长：`886.44` 秒，即 `886440` 毫秒**
- 视频资产 ID：`asset:video-02-05`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "02-04",
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-02-05"`；
- 所有时间戳不超过 `886440`；
