...
- 课次 ID：`05-01`
- 章节：`05 在链表中穿针引线`
- 目录原始标题：`链表，在节点间穿针引线 Reverse Linked List`
- 目录规范标题：`链表，在节点间穿针引线 Reverse Linked List`
- 目录与 FFprobe 时长：`15:44（目录标称）；947.52 秒（FFprobe）`
- **视频实测总时长：`947.52` 秒，即 `947520` 毫秒**
- 视频资产 ID：`asset:video-05-01`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "05-02",
    "title": "测试你的链表程序"
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-05-01"`；
- 所有时间戳不超过 `947520`；
