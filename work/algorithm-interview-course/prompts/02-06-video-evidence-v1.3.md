...
- 课次 ID：`02-06`
- 章节：`02 面试中的复杂度分析`
- 目录原始标题：`均摊时间复杂度分析（Amortized Time Analysis）`
- 目录规范标题：`均摊时间复杂度分析（Amortized Time Analysis）`
- 目录与 FFprobe 时长：`15:16（目录标称）；916.28 秒（FFprobe）`
- **视频实测总时长：`916.28` 秒，即 `916280` 毫秒**
- 视频资产 ID：`asset:video-02-06`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "02-05",
    "title": "递归算法的复杂度分析"
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-02-06"`；
- 所有时间戳不超过 `916280`；
