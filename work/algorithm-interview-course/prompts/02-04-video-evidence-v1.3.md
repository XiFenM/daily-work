...
- 课次 ID：`02-04`
- 章节：`02 面试中的复杂度分析`
- 目录原始标题：`亲自试验自己算法的时间复杂度`
- 目录规范标题：`亲自试验自己算法的时间复杂度`
- 目录与 FFprobe 时长：`14:53（目录标称）；893.48 秒（FFprobe）`
- **视频实测总时长：`893.48` 秒，即 `893480` 毫秒**
- 视频资产 ID：`asset:video-02-04`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "02-03",
    "title": "简单的复杂度分析"
...
7. 非题解课允许 `problem: null`，不强行生成题面、解法或证明。
8. `provenance.requestId`、`provenance.generatedAt` 固定为 `null`，由本地管线注入。
9. 关系候选由课程明确承接或给定候选提出；编辑推断使用 `editorial_hypothesis`。
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-02-04"`；
- 所有时间戳不超过 `893480`；
