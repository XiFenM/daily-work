...
- 课次 ID：`04-07`
- 章节：`04 查找表相关问题`
- 目录原始标题：`查找表和滑动窗口 Contain Duplicate II`
- 目录规范标题：`查找表和滑动窗口 Contain Duplicate II`
- 目录与 FFprobe 时长：`11:20（目录标称）；680.484 秒（FFprobe）`
- **视频实测总时长：`680.484` 秒，即 `680484` 毫秒**
- 视频资产 ID：`asset:video-04-07`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "04-06",
    "title": "灵活选择键值 Number of Boomerangs"
  },
  {
    "lessonId": "04-08",
    "title": "二分搜索树底层实现的顺序性 Contain Duplicate III"
  }
]`
- 可核查的关系候选：`[
  {
  ...
  {
    "from": "lesson:04-07",
    "fromTitle": "查找表和滑动窗口 Contain Duplicate II",
    "to": "lesson:04-08",
    "toTitle": "二分搜索树底层实现的顺序性 Contain Duplicate III",
    "type": "extends",
    "status": "provisional",
    "priorKind": "non_evidence_candidate_for_checking"
  }
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-04-07"`；
- 所有时间戳不超过 `680484`；
