...
- 课次 ID：`08-04`
- 章节：`08 递归和回溯法`
- 目录原始标题：`组合问题 Combinations`
- 目录规范标题：`组合问题 Combinations`
- 目录与 FFprobe 时长：`14:27（目录标称）；867.84 秒（FFprobe）`
- **视频实测总时长：`867.84` 秒，即 `867840` 毫秒**
- 视频资产 ID：`asset:video-08-04`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "08-03",
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-08-04"`；
- 所有时间戳不超过 `867840`；
