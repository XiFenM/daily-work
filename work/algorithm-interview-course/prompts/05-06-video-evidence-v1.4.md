...
- 课次 ID：`05-06`
- 章节：`05 在链表中穿针引线`
- 目录原始标题：`链表与双指针 Remove Nth Node Form End of List`
- 目录规范标题：`链表与双指针 Remove Nth Node Form End of List`
- 目录与 FFprobe 时长：`16:49（目录标称）；1013.52 秒（FFprobe）`
- **视频实测总时长：`1013.52` 秒，即 `1013520` 毫秒**
- 视频资产 ID：`asset:video-05-06`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "05-05",
    "title": "不仅仅是穿针引线 Delete Node in a Linked List"
  }
]`
- 可核查的关系候选：`[
  {
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-05-06"`；
- 所有时间戳不超过 `1013520`；
