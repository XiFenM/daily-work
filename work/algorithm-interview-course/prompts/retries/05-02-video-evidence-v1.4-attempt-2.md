...
- 目录原始标题：`测试你的链表程序`
- 目录规范标题：`测试你的链表程序`
- 目录与 FFprobe 时长：`16:26（目录标称）；986.96 秒（FFprobe）`
- **视频实测总时长：`986.96` 秒，即 `986960` 毫秒**
- 视频资产 ID：`asset:video-05-02`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "05-01",
    "title": "链表，在节点间穿针引线 Reverse Linked List"
  },
  {
    "lessonId": "05-03",
    "title": "设立链表的虚拟头结点 Remove Linked List Elements"
  }
]`
- 可核查的关系候选：`无`
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-05-02"`；
- 所有时间戳不超过 `986960`；
...
上一次响应因 `relationCandidates[1].type` 使用未定义值 `precedes` 被本地 canonical schema 拒绝。请重新分析视频并生成完整 JSON，不要机械复制上一次输出。

- `relationCandidates[].type` 只能是：`prerequisite_of`、`recommended_before`、`extends`、`revisits`、`applies`、`contrasts_with`、`alternative_to`、`same_pattern_as`、`same_problem_family_as`。
- 如果候选关系无法准确映射到上述类型，直接省略该候选，不要创造近义枚举。
- 特别检查屏幕函数名和类型名的每个字符；若 `ListNode` 等代码字符不清晰，应使用 `partial/uncertain` 和 `ocrUncertainties`，不得把疑似 OCR 缺字标成完整代码。
