...
- 课次 ID：`04-08`
- 章节：`04 查找表相关问题`
- 目录原始标题：`二分搜索树底层实现的顺序性 Contain Duplicate III`
- 目录规范标题：`二分搜索树底层实现的顺序性 Contain Duplicate III`
- 目录与 FFprobe 时长：`10:50（目录标称）；654.28 秒（FFprobe）`
- **视频实测总时长：`654.28` 秒，即 `654280` 毫秒**
- 视频资产 ID：`asset:video-04-08`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "04-07",
    "title": "查找表和滑动窗口 Contain Duplicate II"
  }
]`
- 可核查的关系候选：`[
  {
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-04-08"`；
- 所有时间戳不超过 `654280`；
