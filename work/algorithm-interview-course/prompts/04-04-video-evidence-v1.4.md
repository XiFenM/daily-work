...
- 课次 ID：`04-04`
- 章节：`04 查找表相关问题`
- 目录原始标题：`使用查找表的经典问题 Two Sum`
- 目录规范标题：`使用查找表的经典问题 Two Sum`
- 目录与 FFprobe 时长：`16:56（目录标称）；1017 秒（FFprobe）`
- **视频实测总时长：`1017` 秒，即 `1017000` 毫秒**
- 视频资产 ID：`asset:video-04-04`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "04-03",
...
- 可核查的关系候选：`[
  ...
  {
    "from": "lesson:04-04",
    "fromTitle": "使用查找表的经典问题 Two Sum",
    "to": "lesson:04-05",
    "toTitle": "灵活选择键值 4Sum II",
    "type": "extends",
    "status": "provisional",
    "priorKind": "non_evidence_candidate_for_checking"
  }
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-04-04"`；
- 所有时间戳不超过 `1017000`；
