...
- 课次 ID：`02-01`
- 章节：`02 面试中的复杂度分析`
- 目录原始标题：`究竟什么是大O（Big O）`
- 目录规范标题：`究竟什么是大 O（Big O）`
- 目录与 FFprobe 时长：`19:52（目录标称）；1195.6 秒（FFprobe）`
- **视频实测总时长：`1195.6` 秒，即 `1195600` 毫秒**
- 视频资产 ID：`asset:video-02-01`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "02-02",
    "title": "对数据规模有一个概念"
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-02-01"`；
- 所有时间戳不超过 `1195600`；
