...
- 课次 ID：`03-07`
- 章节：`03 数组中的问题其实最常见`
- 目录原始标题：`滑动窗口 Minimum Size Subarray Sum`
- 目录规范标题：`滑动窗口 Minimum Size Subarray Sum`
- 目录与 FFprobe 时长：`14:09（目录标称）；849.92 秒（FFprobe）`
- **视频实测总时长：`849.92` 秒，即 `849920` 毫秒**
- 视频资产 ID：`asset:video-03-07`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "03-06",
  ...
  {
    "lessonId": "03-08",
    "title": "在滑动窗口中做记录 Longest Substring Without Repeating Characters"
  }
]`
- 可核查的关系候选：`[
  {
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-03-07"`；
- 所有时间戳不超过 `849920`；
