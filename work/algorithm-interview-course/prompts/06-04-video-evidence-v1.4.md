...
- 课次 ID：`06-04`
- 章节：`06 栈，队列，优先队列`
- 目录原始标题：`队列的典型应用 Binary Tree Level Order Traversal`
- 目录规范标题：`队列的典型应用 Binary Tree Level Order Traversal`
- 目录与 FFprobe 时长：`10:26（目录标称）；626.32 秒（FFprobe）`
- **视频实测总时长：`626.32` 秒，即 `626320` 毫秒**
- 视频资产 ID：`asset:video-06-04`
- 相邻课次，仅用于定位：`[
  {
    "lessonId": "06-03",
    "title": "运用栈模拟递归"
  ...
  {
    "lessonId": "06-05",
    "title": "BFS 和图的最短路径 Perfect Squares"
  }
]`
- 可核查的关系候选：`[
  {
...
- 所有证据 ID 存在；
- 所有 evidence 使用 `assetId: "asset:video-06-04"`；
- 所有时间戳不超过 `626320`；
